...
UFDT UFDTArr[MAXINODE]; 	//Array of UFDT of elements = MAXINODE ie. 50
...
void DisplayHelp()
{
	...
	printf("fstat : To Display information of file using the file Descriptor\n");
...
void ls_File()
{
	...
	{
		printf("Error : There ere no files\n");
		return;
	}
...
int main()
{
	...
	InitialiseSuperBlock();		//Auxillary data(constructor sarkh)
	CreateDILB();				//Auxillary data(constructor sarkh)
	...
	while(1)
	{
		...
		count = sscanf(str,"%s%s%s%s",command[0],command[1],command[2],command[3]);		//tukde padle//tokanization
		...
		else if(count == 4)
		{
			if(strcmp(command[0],"lseek") == 0)
			{
				...
				if(ret == -1)
				{
					printf("ERROR : Unavalible to perform lseek\n");
				}
			}
			else
			{
